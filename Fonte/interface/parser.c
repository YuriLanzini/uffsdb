...
{
    pthread_t pth;
    char command_buffer[BUFFER_SIZE]; // Buffer maior para acumular comandos de múltiplas linhas
    ...
    while (1)
    {
        if (GLOBAL_PARSER.consoleFlag == 1)
        {
            if (!connected.conn_active)
            {
                printf(">");
        ...
        while ((c = getchar()) != EOF)
        {
            ...
            if (c == '\n' && GLOBAL_PARSER.consoleFlag == 0 && command_len > 0)
            {
                printf("%s-# ", connected.db_name);
            }
            ...
            if (c == '\\')
            {
                ...
                while ((c = getchar()) != EOF && c != '\n')
                {
                    ...
                    if (c == 'c' || c == 't')
                    {
                        // Leitura do nome do banco após \c
                        while ((c = getchar()) != EOF && c != '\n')
                        {
                            command_buffer[command_len++] = c;
                        }
                ...
                if (strcmp(command_buffer, "\\h") == 0 || strcmp(command_buffer, "\\d") == 0 || strcmp(command_buffer, "\\l") == 0 || strcmp(command_buffer, "\\q") == 0)
                {
                ...
                if (!valid_command)
                {
                    invalidCommand(command_buffer);
                    GLOBAL_PARSER.consoleFlag = 1;
                }
                break;
            }

            if (strcmp(command_buffer, "help") == 0 || strcmp(command_buffer, "clear") == 0 || strcmp(command_buffer, "contributors") == 0)
            {
        ...
        // Limpar buffer de comandos após execução
        command_len = 0;
        memset(command_buffer, 0, sizeof(command_buffer));
    }
    return 0;
